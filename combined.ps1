function Get-ClusterCapacityCheck {
    ...
    $ClusterCPUUsageMHz = ($VMHost | Get-Datastore | Where-Object {$_.Extensiondata.Summary.MultipleHostAccess -eq $True} | Measure-Object -Property CpuUsageMhz -Sum).Sum
    $ClusterCPUTotalMHz = ($VMHost | Get-Datastore | Where-Object {$_.Extensiondata.Summary.MultipleHostAccess -eq $True} | Measure-Object -Property CpuTotalMhz -Sum).Sum
    ...
    Get-Cluster | Get-ClusterCapacityCheck | Select-Object Cluster,ClusterFreeDiskspaceGB,ClusterCPUUsageMHz,ClusterCPUTotalMHz
